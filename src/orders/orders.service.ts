...
export class OrdersService {
  constructor (@InjectRepository(OrderEntity) private readonly orderRepository:Repository<OrderEntity> ,
    @InjectRepository(OrdersProductsEntity) private readonly ordersProductsRepository:Repository<OrdersProductsEntity>
  ...
  async  create(createOrderDto: CreateOrderDto,currentUser:UserEntity) : Promise<OrderEntity> {
    ...
    for(let i=0;i<createOrderDto.ordersProducts.length;i++)
    {
      ...
      const product=await this.productsService.findOne(createOrderDto.ordersProducts[i].id);
      ...
      opEntity.push({order,product,product_unit_price,product_quantity})
    }
    const op = await this.orderRepository.createQueryBuilder().insert().into(OrdersProductsEntity).values(opEntity).execute();
    return await this.findOne(orderTbl.id) ;
  }

  async findAll() : Promise<OrderEntity[]> {
    return await this.orderRepository.find({relations:{user:true,shippingAdress:true,products:{product:true}}}); 
  }

 async findOne(id: number) : Promise<OrderEntity> {
    return await this.orderRepository.findOne({where:{id:id},relations:{user:true,shippingAdress:true,products:{product:true}}}); 
  }

  async findOneByProduct(id: number) {
    return await this.ordersProductsRepository.findOne({relations:{product:true},where:{product:{id:id}}});
  }
